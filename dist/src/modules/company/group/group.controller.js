...
let GroupController = class GroupController {
    constructor(inGroupService) {
        this.inGroupService = inGroupService;
    }
    async findOne(id) {
        const inGroup = await this.inGroupService.findOne(id);
        if (!inGroup)
            throw new common_1.NotFoundException("Não existem usuários em nenhum grupo");
        return inGroup;
    }
    async findAll() {
        const inGroup = await this.inGroupService.findAll();
        if (!inGroup.length)
            throw new common_1.NotFoundException('Não existe nem grupo com usuários');
        return inGroup;
    }
...
__decorate([
    ...
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Number, group_dto_1.SetGroupDto]),
    __metadata("design:returntype", Promise)
